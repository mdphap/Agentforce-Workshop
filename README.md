...
- Recommend ingredients and products (especially thoes that our shop offers 😉).
...
## Workshop pre-works
Complete these tasks prior to workshop day so we can have a clean and smooth start:
- **Turn on Data Cloud**: provision Data 360 for your org by following steps at [https://help.salesforce.com/s/articleView?id=data.c360_a_setup_provision.htm&type=5](https://help.salesforce.com/s/articleView?id=data.c360_a_setup_provision.htm&type=5).
- **Turn on Einstein Generative AI**: enable Einstein Generative AI follow steps at [https://help.salesforce.com/s/articleView?id=ai.generative_ai_enable.htm&type=5](https://help.salesforce.com/s/articleView?id=ai.generative_ai_enable.htm&type=5). Skip steps 3 and 4 in the aforementioned article.
- **Enable Agentforce**: enable Agentforce in your DE org by following steps at [https://help.salesforce.com/s/articleView?id=ai.agent_setup_enable.htm&type=5](https://help.salesforce.com/s/articleView?id=ai.agent_setup_enable.htm&type=5)
- **Enable Person Account**: enable Person Account in your DE org by following steps at [https://help.salesforce.com/s/articleView?id=sales.account_person_enable.htm&type=5](https://help.salesforce.com/s/articleView?id=sales.account_person_enable.htm&type=5). Note that you might be asked to create an Account record type as prerequisite of enabling Person Account.
- **Enable Enhanced Chat**: enable Enhanced Chat by going to `Setup` -> search for `Messaging Settings` -> flip `Messaging` toggle on.
- **Enable Digital Experiences**: enable Digital Experiences to support Enhanced Chat web deployment by following steps at [https://help.salesforce.com/s/articleView?id=experience.networks_enable.htm&type=5](https://help.salesforce.com/s/articleView?id=experience.networks_enable.htm&type=5).
...
- **Agent Action**: executable functions assigned to `Agent` and can be used as `Tools`.
...
- **Enhanced Chat**: modernized version of Salesforce Chat. It's the backbone for Service Cloud text messaging channel.

## Workshop Exercises
### Exercise 1: Create Agent User
...
| Profile | Einstein Agent User | Although Agentforce documentation recommended to clone Einstein Agent User profile into a custom one before assigning to Agent User, from our experiment Agent User with custom profile won't be selectable in Agentforce Studio |
...
Provide your own bundle name and API name. We recommend keeping the API name as the default suggested value.
...
### Exercise 4: Create subagent Query Product
...
- **Functional Requirement**:
    ...
    - Don't outright refuse the query if product name is incorrect or having typo. `Agent` would be able to find a reasonable product with similar name and suggest user if it's the correct one. For example: `any fresh tomatoos mate?` should be replied with information about `tomotoes` or ask customer if `tomatoes` is the one they are mentioning.
...
- **Functional Requirement**:
    ...
    - Our products are divisible, `Agent` should allow customer to buy a fraction of a product. For example: `get me a quarter of kg of Tomato` should be accepted and add 0.25 kg Tomato to shopping cart.
...
### Exercise 6: Create cart checkout subagent
...
### Exercise 7: Create general culinary knowledge subagent
...
- **Functional Requirement**:
    - Have small talks with customer about general culinary knowledge.
    - Depending on the context, the reply should be relevant to customer selected product or our offered product. For example: `how can I best prepare this salmon for dinner?` should provide the culinary knowledge answer and suggest we have `salt` or `pepper` that might fit to `salmon preparation for dinner`.

- **Exercise tips**:
    - Refer shop products and shopping cart in the reasoning instruction.

### Exercise 8: Publish & Activate Agent
...
After publishing, activate the `Agent` In `Agentforce Studio`.
...
### Exercise 9: Test Agent
The non-deterministic nature of conversation poses a unique challenge for `Agent` testing and require a shift in mindset & tools. `Agentforce Testing Center` built specifically to test `Agent` and support wide range of tools for autonomous agent testing.
 
For our workshop, perform the following tests with our `Agent`:
| Test Category | Test Suite | Details |
| -------- | -------- | -------- |
| Subagent Router, Tool Usage | Query Product | Use single utterance to evaluate `Agent` can route to Query Product subagent |
...
### Exercise 10: Employ Agent to Enhanced Chat
...
5. Update `store/index.html` with your messaging channel configurations.

### Exercise 11: Monitor Agent performance and feedback
...
[Agent Analytics](https://help.salesforce.com/s/articleView?id=ai.generative_ai_agent_analytics.htm&type=5)

[Agentforce Session Tracing](https://help.salesforce.com/s/articleView?id=ai.generative_ai_session_trace.htm&type=5)

### Exercise 12: Deploy Agent to Other Org
